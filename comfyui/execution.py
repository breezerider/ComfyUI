...
from comfyui.comfy_execution.graph import get_input_info, ExecutionList, DynamicPrompt, ExecutionBlocker
from comfyui.comfy_execution.graph_utils import is_link, GraphBuilder
from comfyui.comfy_execution.caching import HierarchicalCache, LRUCache, CacheKeySetInputSignature, CacheKeySetID
...
class IsChangedCache:
    ...
    def get(self, node_id):
        ...
        class_def = comfyui.nodes.NODE_CLASS_MAPPINGS[class_type]
...
def _map_node_over_list(obj, input_data_all, func, allow_interrupt=False, execution_block_cb=None, pre_execute_cb=None):
    ...
    def process_inputs(inputs, index=None):
        if allow_interrupt:
            comfyui.nodes.before_node_execution()
...
def execute(server, dynprompt, caches, current_item, extra_data, executed, prompt_id, execution_list, pending_subgraph_results):
    ...
    try:
    ...
    except comfyui.comfy.model_management.InterruptProcessingException as iex:
        ...
        if isinstance(ex, comfyui.comfy.model_management.OOM_EXCEPTION):
...
class PromptExecutor:
    ...
    def handle_execution_error(self, prompt_id, prompt, current_outputs, executed, error, ex):
        ...
        if isinstance(ex, comfyui.comfy.model_management.InterruptProcessingException):
            mes = {
                "prompt_id": prompt_id,
                "node_id": node_id,
                "node_type": class_type,
    ...
    def execute(self, prompt, prompt_id, extra_data={}, execute_outputs=[]):
        comfyui.nodes.interrupt_processing(False)
        ...
        with torch.inference_mode():
            ...
            comfyui.comfy.model_management.cleanup_models(keep_clone_weights_loaded=True)
            ...
            if comfyui.comfy.model_management.DISABLE_SMART_MEMORY:
                comfyui.comfy.model_management.unload_all_models()
...
def validate_inputs(prompt, item, validated):
    ...
    obj_class = comfyui.nodes.NODE_CLASS_MAPPINGS[class_type]
    ...
    for x in valid_inputs:
        ...
        if isinstance(val, list):
            ...
            r = comfyui.nodes.NODE_CLASS_MAPPINGS[o_class_type].RETURN_TYPES
...
def validate_prompt(prompt):
    ...
    for x in prompt:
        ...
        class_ = comfyui.nodes.NODE_CLASS_MAPPINGS.get(class_type, None)
        if class_ is None:
            error = {
                "type": "invalid_prompt",
