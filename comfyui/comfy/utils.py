...
import math
import struct
import comfyui.comfy.checkpoint_pickle
...
def load_torch_file(ckpt, safe_load=False, device=None):
    ...
    if ckpt.lower().endswith(".safetensors") or ckpt.lower().endswith(".sft"):
        ...
        if safe_load:
            ...
            pl_sd = torch.load(ckpt, map_location=device, pickle_module=comfyui.comfy.checkpoint_pickle)
...
def reshape_mask(input_mask, output_shape):
    ...
    mask = comfyui.comfy.utils.repeat_to_batch_size(mask, output_shape[0])
    return mask
