...
from comfyui.comfy.k_diffusion import sampling as k_diffusion_sampling
...
class BasicScheduler:
    ...
    def INPUT_TYPES(s):
        return {"required":
                    {"model": ("MODEL",),
                     "scheduler": (comfyui.comfy.samplers.SCHEDULER_NAMES, ),
    ...
    def get_sigmas(self, model, scheduler, steps, denoise):
        ...
        sigmas = comfyui.comfy.samplers.calculate_sigmas(model.get_model_object("model_sampling"), scheduler, total_steps).cpu()
...
class BetaSamplingScheduler:
    ...
    def get_sigmas(self, model, steps, alpha, beta):
        sigmas = comfyui.comfy.samplers.beta_scheduler(model.get_model_object("model_sampling"), steps, alpha=alpha, beta=beta)
        return (sigmas, )
...
class KSamplerSelect:
    ...
    def INPUT_TYPES(s):
        return {"required":
                    {"sampler_name": (comfyui.comfy.samplers.SAMPLER_NAMES, ),
                      }
               }
    ...
    def get_sampler(self, sampler_name):
        sampler = comfyui.comfy.samplers.sampler_object(sampler_name)
        return (sampler, )
...
class SamplerDPMPP_3M_SDE:
    ...
    def get_sampler(self, eta, s_noise, noise_device):
        ...
        sampler = comfyui.comfy.samplers.ksampler(sampler_name, {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerDPMPP_2M_SDE:
    ...
    def get_sampler(self, solver_type, eta, s_noise, noise_device):
        ...
        sampler = comfyui.comfy.samplers.ksampler(sampler_name, {"eta": eta, "s_noise": s_noise, "solver_type": solver_type})
        return (sampler, )
...
class SamplerDPMPP_SDE:
    ...
    def get_sampler(self, eta, s_noise, r, noise_device):
        ...
        sampler = comfyui.comfy.samplers.ksampler(sampler_name, {"eta": eta, "s_noise": s_noise, "r": r})
        return (sampler, )
...
class SamplerDPMPP_2S_Ancestral:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = comfyui.comfy.samplers.ksampler("dpmpp_2s_ancestral", {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerEulerAncestral:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = comfyui.comfy.samplers.ksampler("euler_ancestral", {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerEulerAncestralCFGPP:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = comfyui.comfy.samplers.ksampler(
...
class SamplerLMS:
    ...
    def get_sampler(self, order):
        sampler = comfyui.comfy.samplers.ksampler("lms", {"order": order})
        return (sampler, )
...
class SamplerDPMAdaptative:
    ...
    def get_sampler(self, order, rtol, atol, h_init, pcoeff, icoeff, dcoeff, accept_safety, eta, s_noise):
        sampler = comfyui.comfy.samplers.ksampler("dpm_adaptive", {"order": order, "rtol": rtol, "atol": atol, "h_init": h_init, "pcoeff": pcoeff,
...
class Noise_RandomNoise:
    ...
    def generate_noise(self, input_latent):
        ...
        return comfyui.comfy.sample.prepare_noise(latent_image, self.seed, batch_inds)
...
class SamplerCustom:
    ...
    def sample(self, model, add_noise, noise_seed, cfg, positive, negative, sampler, sigmas, latent_image):
        ...
        x0_output = {}
        callback = comfyui.latent_preview.prepare_callback(model, sigmas.shape[-1] - 1, x0_output)

        disable_pbar = not comfyui.comfy.utils.PROGRESS_BAR_ENABLED
        samples = comfyui.comfy.sample.sample_custom(model, noise, cfg, sampler, sigmas, positive, negative, latent_image, noise_mask=noise_mask, callback=callback, disable_pbar=disable_pbar, seed=noise_seed)
...
class Guider_Basic(comfyui.comfy.samplers.CFGGuider):
...
class CFGGuider:
    ...
    def get_guider(self, model, positive, negative, cfg):
        guider = comfyui.comfy.samplers.CFGGuider(model)
        guider.set_conds(positive, negative)
        guider.set_cfg(cfg)
        return (guider,)

class Guider_DualCFG(comfyui.comfy.samplers.CFGGuider):
    def set_cfg(self, cfg1, cfg2):
        self.cfg1 = cfg1
        self.cfg2 = cfg2

    def set_conds(self, positive, middle, negative):
        middle = comfyui.node_helpers.conditioning_set_values(middle, {"prompt_type": "negative"})
    ...
    def predict_noise(self, x, timestep, model_options={}, seed=None):
        ...
        out = comfyui.comfy.samplers.calc_cond_batch(self.inner_model, [negative_cond, middle_cond, self.conds.get("positive", None)], x, timestep, model_options)
        return comfyui.comfy.samplers.cfg_function(self.inner_model, out[1], out[0], self.cfg2, x, timestep, model_options=model_options, cond=middle_cond, uncond=negative_cond) + (out[2] - out[1]) * self.cfg1
...
class SamplerCustomAdvanced:
    ...
    def sample(self, noise, guider, sampler, sigmas, latent_image):
        ...
        latent_image = latent["samples"]
        latent = latent.copy()
        latent_image = comfyui.comfy.sample.fix_empty_latent_channels(guider.model_patcher, latent_image)
        latent["samples"] = latent_image
        ...
        x0_output = {}
        callback = comfyui.latent_preview.prepare_callback(guider.model_patcher, sigmas.shape[-1] - 1, x0_output)

        disable_pbar = not comfyui.comfy.utils.PROGRESS_BAR_ENABLED
        samples = guider.sample(noise.generate_noise(latent), latent_image, sampler, sigmas, denoise_mask=noise_mask, callback=callback, disable_pbar=disable_pbar, seed=noise.seed)
        samples = samples.to(comfyui.comfy.model_management.intermediate_device())
