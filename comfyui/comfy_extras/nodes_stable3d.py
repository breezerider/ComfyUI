...
class StableZero123_Conditioning:
    ...
    def INPUT_TYPES(s):
        return {"required": { "clip_vision": ("CLIP_VISION",),
                              "init_image": ("IMAGE",),
                              "vae": ("VAE",),
                              "width": ("INT", {"default": 256, "min": 16, "max": comfyui.nodes.MAX_RESOLUTION, "step": 8}),
                              "height": ("INT", {"default": 256, "min": 16, "max": comfyui.nodes.MAX_RESOLUTION, "step": 8}),
...
class StableZero123_Conditioning_Batched:
    ...
    def encode(self, clip_vision, init_image, vae, width, height, batch_size, elevation, azimuth, elevation_batch_increment, azimuth_batch_increment):
        ...
        cond = torch.cat([comfyui.comfy.utils.repeat_to_batch_size(pooled, batch_size), cam_embeds], dim=-1)
...
class SV3D_Conditioning:
    ...
    def INPUT_TYPES(s):
        return {"required": { "clip_vision": ("CLIP_VISION",),
                              "init_image": ("IMAGE",),
                              "vae": ("VAE",),
                              "width": ("INT", {"default": 576, "min": 16, "max": comfyui.nodes.MAX_RESOLUTION, "step": 8}),
